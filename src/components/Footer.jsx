      ...
      <div className="container-lux">
        ...
        <div className="grid lg:grid-cols-[1.4fr_2fr] gap-14 pb-14 border-b border-ivory/10">
          ...
          <div>
            ...
            <VeinLine
              className="h-3 w-24 mt-3 text-bronze-light"
              animate={false}
            />

            <p className="mt-5 text-ivory/55 leading-relaxed max-w-xs">
              {siteConfig.tagline}
            </p>
            ...
            <div className="mt-8 space-y-7">
              {siteConfig.locations.map((location, index) => (
                <div key={location.name}>
                  <p className="text-[10px] uppercase tracking-[0.25em] text-bronze-light mb-2">
                    Location {index + 1} — {location.name}
                  </p>

                  <div className="flex items-start gap-3 text-sm text-ivory/60 leading-relaxed">
                    <MapPin
                      size={16}
                      className="mt-1 shrink-0 text-bronze-light"
                      strokeWidth={1.5}
                    />
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-8">
            {columns.map((col) => (
              <div key={col.title}>
                <p className="text-xs tracking-widest2 uppercase text-ivory/40 mb-4">
                  {col.title}
                </p>
